...
const CastImg = () => {
  ...
  return (
    <div className='overflow-x-hidden bg-black text-white  border-t-2'>
      ...
      <h1 className='text-5xl'> {getMoviesDetails.title}</h1>
      <p className='pb-3'> {getMoviesDetails.tagline}</p>

      <p className='w-[40rem] pb-3 text-2xl'>Overview: {getMoviesDetails.overview}</p>
      <p className='text-[1.5rem]'> Popularity: {getMoviesDetails.popularity}</p>
       <p className='text-[1.5rem]'> Genres: {getMoviesDetails.genres.map((gname)=>gname.name)}</p>
        <p className='text-[1.5rem]'>status: {getMoviesDetails.status}</p>
      </div>
            ...
            if (cast.profile_path) {
              return (
                <div className='' key={cast.id}>
                  ...
                  <div className='w-36 m-2' key={cast.id}>
                    <img className='w-36' alt='castImg' key={cast.id} src={IMG_URL + cast.profile_path} />
